...
function readJson(path: string): JsonObject {
  ...
  try {
    ...
    if (code === 'ENOENT') throw new Error(`missing canonical file: ${path}`)
...
export function loadWorkspace(output: string): { tutorial: JsonObject; state: JsonObject } {
...
export function statusSummary(tutorial: JsonObject, state: JsonObject): JsonObject {
  const titles = Object.fromEntries(tutorial.chapterPlan.map((item: JsonObject) => [item.id, item.title]))
  const read = state.chapters.filter((item: JsonObject) => item.status === 'read').map((item: JsonObject) => item.id)
  const unresolvedQuestions: JsonObject[] = []
  const records = state.memory?.chapterRecords
  if (records && typeof records === 'object' && !Array.isArray(records)) {
    for (const [chapterId, record] of Object.entries(records) as [string, JsonObject][]) {
...
function appendMemory(record: JsonObject, key: string, values: string[]): void {
  const target = (record[key] ??= [])
  if (!Array.isArray(target)) throw new Error(`memory record ${key} must be an array`)
  target.push(...values.map((text) => ({ text, source: 'learner', at: utcNow() })))
}

function eventsOf(state: JsonObject): JsonObject[] {
...
export function completeCurrent(
  ...
  if (typeof currentId !== 'string') throw new Error('learning state has no current chapter')
  if (!Array.isArray(state.chapters)) throw new Error('learning-state chapters must be an array')
  const currentIndex = state.chapters.findIndex((item: JsonObject) => item.id === currentId)
...
export function remember(state: JsonObject, chapterId: string | undefined, input: MemoryInput): void {
