...
function renderPriorApproaches(approaches) {
    return `<div class="approach-grid">${approaches.map((approach) => `<article class="approach-card"><h4>${escaped(approach.name)}</h4><p><strong>Contribution.</strong> ${escaped(approach.contribution)}</p><p><strong>Binding limitation.</strong> ${escaped(approach.limitation)}</p></article>`).join('')}</div>`;
}
...
export function renderHtml(data, state) {
    const generatedIds = new Set(data.chapters.map((chapter) => chapter.id));
    const statuses = Object.fromEntries(state.chapters.map((item) => [item.id, item.status]));
    const navigation = data.chapterPlan.map((item, index) => generatedIds.has(item.id)
        ? `<li><a href="#${escaped(item.id)}">${index + 1}. ${escaped(item.title)} <small>${escaped(statuses[item.id])}</small></a></li>`
        : `<li class="planned-chapter"><span>${index + 1}. ${escaped(item.title)}</span><small>${escaped(statuses[item.id])}</small></li>`).join('');
    const current = data.chapterPlan.find((item) => item.id === state.currentChapterId);
    const currentTitle = current?.title ?? 'Textbook complete';
    const completedCount = state.chapters.filter((item) => item.status === 'read').length;
    ...
    const lineage = data.chapterPlan.map((item, index) => generatedIds.has(item.id)
        ...
        : `<li class="planned-chapter"><span>${index + 1}</span><strong>${escaped(item.title)}</strong><small>${escaped(item.period)} · ${escaped(statuses[item.id])}</small></li>`).join('');
    const rubricRows = data.doctoralRubric.map((item) => `<tr><th scope="row">${escaped(item.capability)}</th><td>${escaped(item.standard)}</td></tr>`).join('');
...
<body>
  ...
  <div class="layout">
    ...
    <main id="main-content">
      ...
      ${data.chapters.map((chapter, index) => renderChapter(chapter, index + 1)).join('')}
      ${finalSections}
    </main>
  </div>
</body>
...
export function validate(data) {
    ...
    if (!qualification || typeof qualification !== 'object' || Array.isArray(qualification)) {
        ...
        for (const key of ['requestedDirection', 'decision', 'admittedResearchField', 'selectionMethod', 'rationale']) {
            if (!isNonEmptyString(qualification[key]))
                errors.push(`fieldQualification.${key} must be a non-empty string`);
        }
        if (!['accepted', 'reframed', 'narrowed'].includes(qualification.decision))
            errors.push('fieldQualification.decision is invalid');
        if (!['accepted-as-stated', 'chosen-by-user'].includes(qualification.selectionMethod))
            errors.push('fieldQualification.selectionMethod is invalid');
        ...
        if (qualification.admittedResearchField !== value.field)
            errors.push('field must equal fieldQualification.admittedResearchField');
    }
    ...
    if (!practice || typeof practice !== 'object' || Array.isArray(practice)) {
        ...
        if (!isNonEmptyString(practice.frontierContribution))
            errors.push('researchPractice.frontierContribution must be a non-empty string');
    }
    const planIds = [];
    if (Array.isArray(value.chapterPlan))
        value.chapterPlan.forEach((item, index) => {
            ...
            const plan = item;
            for (const key of ['id', 'title', 'period', 'transitionSummary'])
                if (!isNonEmptyString(plan[key]))
                    errors.push(`${path}.${key} must be a non-empty string`);
            if (typeof plan.id === 'string' && !ID_PATTERN.test(plan.id))
                errors.push(`${path}.id must use lowercase kebab-case`);
            if (planIds.includes(plan.id))
                errors.push(`duplicate chapterPlan id: ${plan.id}`);
            ...
            for (const dependency of Array.isArray(plan.dependsOn) ? plan.dependsOn : []) {
                if (!planIds.includes(dependency))
                    errors.push(`${path}.dependsOn must reference an earlier chapter`);
            }
    ...
    if (Array.isArray(value.chapters))
        value.chapters.forEach((item, index) => {
            ...
            if (!item || typeof item !== 'object' || Array.isArray(item))
                return errors.push(`${path} must be an object`);
            const chapter = item;
            ...
            for (const key of ['limitationsAndDebates', 'downstreamInfluence', 'learningTasks'])
                validateStringArray(chapter[key], `${path}.${key}`, errors);
            if (!Array.isArray(chapter.priorApproaches) || !chapter.priorApproaches.length)
                errors.push(`${path}.priorApproaches must be a non-empty array`);
            else
                chapter.priorApproaches.forEach((approach, approachIndex) => {
                    for (const key of ['name', 'contribution', 'limitation'])
                        if (!isNonEmptyString(approach?.[key]))
                            errors.push(`${path}.priorApproaches item ${approachIndex + 1} requires ${key}`);
                });
            for (const key of ['name', 'idea', 'whyItWorked'])
                if (!isNonEmptyString(chapter.turningPoint?.[key]))
                    errors.push(`${path}.turningPoint requires ${key}`);
            validateWorks(chapter.works, `${path}.works`, errors);
        });
    if (chapterIds.join('\0') !== planIds.slice(0, chapterIds.length).join('\0'))
        errors.push('chapters must be a contiguous prefix of chapterPlan');
    validateWorks(value.bibliography, 'bibliography', errors);
    if (Array.isArray(value.doctoralRubric))
        value.doctoralRubric.forEach((item, index) => {
            for (const key of ['capability', 'standard'])
                if (!isNonEmptyString(item?.[key]))
                    errors.push(`doctoralRubric item ${index + 1} requires ${key}`);
        });
    return errors;
}
function validateWorks(value, path, errors) {
    ...
    value.forEach((item, index) => {
        ...
        const work = item;
        for (const key of ['citation', 'role', 'status'])
            if (!isNonEmptyString(work[key]))
                errors.push(`${path} item ${index + 1} requires ${key}`);
        if (typeof work.status === 'string' && !SOURCE_STATUSES.has(work.status))
            errors.push(`${path} item ${index + 1} has an invalid status`);
        if (work.url) {
            try {
                if (!['http:', 'https:'].includes(new URL(String(work.url)).protocol))
                    throw new Error();
            }
            catch {
...
function initializeState(data) {
    ...
    return {
        ...
        chapters: data.chapterPlan.map((item, index) => ({ id: item.id, status: index === 0 ? 'available' : 'planned', ...(index === 0 ? { generatedAt: now } : {}) })),
        memory: {
            ...
            chapterRecords: Object.fromEntries(data.chapterPlan.map((item) => [item.id, { acknowledgements: [], reflections: [], questions: [], masteryEvidence: [], misconceptions: [], unresolvedQuestions: [], memoryUsed: [] }]))
        },
        events: [{ type: 'chapter-generated', chapterId: firstId, at: now }], updatedAt: now
    };
}
function loadAndSyncState(data, outputPath) {
    ...
    try {
        ...
        throw new Error(`invalid learning state: ${error.message}`);
    }
    if (!state || typeof state !== 'object' || Array.isArray(state))
        throw new Error('learning state root must be an object');
    ...
    if (state.field !== data.field)
        throw new Error('tutorial field conflicts with saved learning state');
    if (!Array.isArray(state.chapters))
        throw new Error('learning state chapters must be an array');
    if (state.chapters.map((item) => item.id).join('\0') !== data.chapterPlan.map((item) => item.id).join('\0'))
        throw new Error('chapterPlan conflicts with saved learning state');
    const generatedIds = new Set(data.chapters.map((chapter) => chapter.id));
    const newlyGenerated = state.chapters.filter((item) => generatedIds.has(item.id) && !item.generatedAt);
    if (newlyGenerated.length > 1 || newlyGenerated.some((item) => item.status !== 'ready'))
        throw new Error('generate exactly the one chapter marked ready');
    if (newlyGenerated.length) {
        ...
        events.push({ type: 'chapter-generated', chapterId: item.id, at: now });
        state.updatedAt = now;
    }
    return state;
}
export function buildTutorial(inputPath, outputPath, force = false) {
    let data;
    try {
    ...
    if (existing.length && !force)
        throw new Error(`managed output already exists (${existing.join(', ')}); pass --force to overwrite`);
